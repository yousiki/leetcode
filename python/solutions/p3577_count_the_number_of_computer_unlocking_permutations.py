...
#
# There are n locked computers in a room with labels from 0 to n - 1, each with
# its own unique password. The password of the computer i has a complexity
# complexity[i].
#
...
# unlocked computer, following this information:
#
#
# You can decrypt the password for the computer i using the password for
# computer j, where j is any integer less than i with a lower complexity. (i.e.
...
class Solution:
  ...
  def countPermutations(self, complexity: list[int]) -> int:
    ...
    if complexity[0] >= min(complexity[1:]):
      return 0
    return reduce(lambda x, y: x * y % self.mod, range(1, len(complexity)))
...
def test_example_1():
  complexity = [1, 2, 3]
  assert Solution().countPermutations(complexity) == 2


def test_example_2():
  complexity = [3, 3, 3, 4, 4, 4]
  assert Solution().countPermutations(complexity) == 0
